...
main() {
    ...
    explain "ClientAliveInterval=300s, ClientAliveCountMax=2 → 600s total timeout"
...
install_dependencies() {
    if ! command -v sshd >/dev/null 2>&1; then
        ...
        record_action "openssh-server installed" "✓"
    else
        log info "SSH server already installed (skipping)"
        record_action "openssh-server already present" "✓"
    fi
}
...
backup_sshd_config() {
    if [ ! -f /etc/ssh/sshd_config.bak ]; then
        ...
        record_action "sshd_config backed up" "💾"
    else
        log info "Backup already exists (skipping)"
        record_action "sshd_config backup exists" "✓"
    fi
...
# Generated by firstb00t module 3 — do not edit manually
...
EOF
    ...
    record_action "Key-based auth enforced, passwords disabled" "🔑"
    record_action "Root login disabled (PermitRootLogin no)" "🚫"
...
# NSA Secure Shell (CSIS 7.x) — Strong Cipher Configuration
...
EOF
    record_action "Strong ciphers configured (AES-256-GCM, ChaCha20)" "🔐"
...
configure_key_exchange() {
    ...
    record_action "KexAlgorithms: Curve25519, DH-GEX-SHA256" "🔐"
}

configure_macs() {
    # Explicitly listed in 99-ciphers-hardened.conf for redundancy
    log info "MAC algorithms hardened: HMAC-SHA2-512-ETM, HMAC-SHA2-256-ETM"
    record_action "MACs: HMAC-SHA2-512-ETM, HMAC-SHA2-256-ETM" "🔐"
}
